...
export const api={
 ...
 attendanceRecords:(classId:string,date:string)=>request<AttendanceRecord[]>(`/api/attendance?classId=${encodeURIComponent(classId)}&date=${encodeURIComponent(date)}`),
 ...
 payments:()=>request<PaymentRecord[]>("/api/payments"),
 ...
 voidPayment:(id:string,reason:string,expectedVersion:number)=>request<{paymentId:string;status:string;version:number;idempotent?:boolean}>(`/api/payments/${encodeURIComponent(id)}`,{method:"DELETE",headers:{"x-request-id":requestId()},body:JSON.stringify({reason,expectedVersion})}),
};
